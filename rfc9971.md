...
The MLRsearch Specification uses a number of specific *quantities*.
...
This document prefers to use computer science vocabulary,
...
Some units might be preferred by some implementations,
...
: Controller implementations might decide to use the same Load in multiple Trials.
...
: Trial Load is the per-interface Intended Load ised in a specific Trial.

Discussion:

: This is a Load-related quantity that is tied to a single Trial,
to distinguish it from later quantities that are not tied that way.
...
except when optionally passing Trial Output instances to the Manager.
...
those Trial Results may be enough for classification as an Upper Bound.
...
the [Load Classification Logic](#load-classification-logic) is based on.
  ...
  Note that this is a limitation for the Trial Duration sected by the Controller.
The Measurer may keep returning different Effective Trial Duration in Trial Results,
...
for reasons why users may want to set this value above zero.
  ...
  Since multiple Trials may be needed for one Load,
...
as in practice that leads to the smallest variation in overall Search Duration.
...
when focusing on the goal.
  ...
  Note that shorter Trial Durations can still be selected by the Controller,
...
Trials with Trial Durations deemed as too unreliable
...
as an ordered list of Goal Results that appears in the exact
sequence of their corresponding Search Goals.

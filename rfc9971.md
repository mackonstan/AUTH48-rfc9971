...
(COTS) CPU hardware vs. purpose-built Application-Specific Integrated Circuit (ASIC) / Network Processing Unit (NPU) / Field-Programmable Gate Array (FPGA) hardware.
...
- Software DUTs show noisy Trial Results,
  leading to a big spread of throughputs that could be discovered.
- Throughput requires a loss of exactly zero frames, but the industry best practices
  ...
  (see [Y.1564] and test-equipment manuals for more information).
- The definition of throughput is not clear when Trial Results are inconsistent
  (e.g., when successive Trial at a higher Load
  yields smaller Trial Loss Ratio, the classical
  Throughput metric introduced in {{RFC1242}} and narrowed down in {{RFC2544}}
  can no longer be pinned to a single, unambiguous value.)
...
1. Allow multiple Short Trials instead of one long Trial per Load.
   - Optionally, tolerate a percentage of Trial Results with higher Trial Loss Ratio.
2. Allow searching for multiple [Search Goals](#search-goal), with differing Goal Loss Ratios.
...
* If loss is zero, set the lower-bound to the midpoint;
  else, set the upper bound to the midpoint.
...
The Loads currently held in the two variables are called *tightest bounds*,
...
as different benchmarking teams may obtain varying throughputs
...
non-zero Goall Loss Ratio for their Load search.
...
- Because noise-related drops usually arrive in small bursts, their
  ...
  performance; consequently, the Trial Loss Ratio ans an average over the Trial
...
each protocol and application can have different realistic Goal Loss Ratios.
...
Furthermore, allowing users to specify multiple Goal Loss Ratios,
...
{{Section 3 of RFC6349}} for Goal Loss Ratios acceptable for an accurate
...
there can be a subsequent inconsistent non-zero-loss Trial Result.
...
Ideally, there will be a definition of a new metric that both generalizes
...
either elsewhere in the document, or in another previously published document.
Lowercase variants are equally valid,
the capitalization was already applied in earlier section where required.
...
This way, the list of terms is visible in the table of contents.
...
The definitions support multi-goal benchmarks, with single-goal ones as a subset.
...
and does not change nor obsolete any part of {{RFC2544}}.
...
* This specification does not mandate or recommend any single,
  ...
  to the operator of the test procedure, or may be recommended in future publications.

* The internal heuristics or algorithms used by the Controller to select Trial Inputs
  are considered to be a mere implementation details.

* The potential for, and the effects of, interference between different Search Goals
  within a multi-goal search are considered outside the normative scope of this specification.

* Treatment of situations where Offered Load is found to differ noticeably from Intended Load
  is not prescribed, only the obvious "use smaller Max Load" recommendation is mentioned.
...
requirements defined for its corresponding MLRsearch Specification section.
...
For one benchamrk, the Manager calls a Controller once,
and the Controller then invokes the Measurer repeatedly
until the Controller decides it has enough information to return its outputs.
...
The Search Result is the combination of regular or irregular results, one for each Search Goal.
...
or when all remaining Search Goals are proven to have only *Irregular Goal Results*.

### Search Duration

Even if the time it takes the Controller to compute *Trial Input* attributes
for next Trial is negligible, Trials themselves take some time to finish,
and SUT behavior affects how quickly stopping conditions can be satisfied.

For large scale tests where many SUT configurations and Traffic Profiles need benchmarking,
...
There are three considerations worth discussing for such *Search Duration* optimization.

Firstly, as SUT behavior can be probabilistic, so can be the Search Duration of each benchmark.
The quantity to optimize is the expected value of Search Duration,
it should be as short as possible within the intended reliability.

Secondly, Search Duration probability distributions with smaller standard deviations are preferred,
as it makes easier to plan execution of multiple benchmarks one-by-one on the same SUT.

Thirdly, any prior knowledge about SUT behavior can be leveraged into average time gains
...
as lowering Search Duration for those helps with standard deviation, if not also the expected value.

Most of optimization heuristics are outside of the scope of this document,
but many design decisions were affected by generic optimizations found in practice so far.
...
The MLRsearch Specification relies on several specific *quantities*.

In physics, you can compare *magnitude* of scalar quantities,
and the same magnitude can be decomposed in multiple ways
as a product of a *unit* and a *value*.
This document prefers to use the computer science vocabulary,
were the term quantity can describe multiple variables or constants at once,
and one *instance* of a scalar quantity is called a *value*,
with the corresponding unit only mentioned when needed.
Where the context allows, a shorter phrase is used,
for example "Load" instead of "value of the Load quantity".

In general, the MLRsearch Specification does not prescribe particular units to be used,
...
In rare cases, a *prime unit* is mentioned,
usually when comparison across different Traffic Profiles would be otherwise ambiguous.
Some units MAY be preferred by some implementations,
for example when they round the values to integers.
...
value of the duration quantity is expected to be equal to the "final duration" value.
...
: The traffic is sent only in phase c) and received in phases c) and d).
  ...
  It is useful to consider the traffic as sent and received by a tester,
as implicitly defined in {{Section 6 of RFC2544}}.
...
It is RECOMMENDED to interpret the "learning frames" to mean
...
but Controller MAY repeat the same Trial Inputs to get multiple independent Trial Outputs.
...
: Load is equivalent to the quantities defined
...
As neither of those older definitions list all properties relevant
for MLRsearch specification, the new word Load was chosen.

  In the common case of bidirectional traffic, as described in
Bidirectional Traffic ({{Section 14 of RFC2544}}),
Load is the Data Rate per direction, half of the aggregate Data Rate.
...
units. Each reported Load MUST unambiguously state whether
...
links the per-interface and multi-interface Load values.
  ...
  The per-interface Load remains the primary unit, consistent
...
MAY report Load-related values as "bidirectional load"
(double of "unidirectional load" if that is distinguished in the same Test Report).
...
only generates the accepted values.
...
: Trial Load is the per-interface Intended Load used in a specific Trial.

Discussion:

: This is a Load-related quantity that is tied to a single Trial,
to distinguish it from later derived quantities that are not tied that way.
...
as long as the intuition of a load intended against a single interface can be applied.
  ...
  It MAY be possible to use a Trial Load to describe non-constant traffic
...
(and frequently undocumented) in practice.
...
that would not be possible to achieve using any integer number of data frames.
  ...
  Given that the Trial Load is a quantity based on Load,
Test Report MAY express this quantity using multi-interface values,
as the sum of Intended Loads over input interfaces.
...
: Trial Input is a composite quantity, consisting of exactly two attributes:
Trial Duration and Trial Load.

Discussion:

: When talking about a set of Trials, it is common to say *Trial Inputs*
...
: All the attributes MUST be constant during the Search,
...
as long as its impact is functionally constant.

  Implementations of the Manager and the Measurer
MUST be aware of their common set of capabilities well enough,
...
: The Trial Forwarding Ratio is a dimensionless floating point quantity.
Its value MUST range between 0.0 and 1.0, both inclusive.
...
based on averaged frame counts when more than one traffic direction is present.
...
while still preserving the direct relation to the per-interface Load.
...
the Controller MUST use the Trial Duration value from its Trial Input instead.
...
The Controller implementations SHOULD ignore any optional attributes
they are not familiar with,
except when optionally passing selected Trial Output instances to the Manager.
...
implied by Trial Load and Trial Duration.
  ...
  While {{Section 3.5.2 of RFC2285}} requires the Offered Load
...
consisting of the Trial Input and the Trial Output as REQUIRED attributes.
...
coefficients in formulas for Controller Output attributes, which are not defined yet.
...
: This is the minimal value for Trial Duration that should be reached.
The value MUST be positive.

Discussion:

: Certain Trials must reach this minimum Trial Duration before a Load can be
classified as a Lower Bound.

  The Controller MAY choose shorter Trial Durations,
and those Trial Results can be enough for classification as an Upper Bound.
...
the [Load Classification Logic](#load-classification-logic) is based on.
  ...
  Note that this is a soft limitation for the Trial Duration values selected by the Controller.
The Measurer MAY keep returning different Effective Trial Durations in Trial Results,
so Controller implementations MUST be ready to handle such situations.
...
for an example where the possibility of multiple Trials at the same Load is intended.

  A Goal Duration Sum shorter than the Goal Final Trial Duration
...
signals the SUT may be unable to process this Load well enough.
...
for reasons why users may want to set this value above zero.
  ...
  Since multiple Trials might be needed for one Load,
...
: Informally, this controls the ratio of Trial Results exceeding Goal Loss Ratio.
...
The actual full logic is more complicated, as shorter Trials are allowed.
  ...
  For explainability reasons, the RECOMMENDED value for the Goal Exceed Ratio is 0.5 (50%),
and in practice that also leads to the smallest variation in overall Search Duration.
...
: This is a threshold value for deciding whether two Load values are close enough.
...
MUST provide the Controller with other means to control the Search *stopping conditions*.
  ...
  *Absolute Load difference* (in fps) and *relative Load difference* (in percents)
are two popular choices, but implementations MAY choose different ways to specify Goal Width.
...
Refer to ["Performance Variability"](#performance-variability) for more elaboration on the reasoning.
...
: This is the minimal value for the Trial Duration suggested to be used for this goal.
...
when focusing on this Search Goal.

  Note that shorter Trial Durations MAY still be selected by the Controller,
for example when focusing on a different Search Goal.
...
MUST be able to handle possibly lower Effective Trial Duration values
returned by the Measurer.
...
Loss Ratio, and Goal Exceed Ratio.
  ...
  Example OPTIONAL attributes are Goal Initial Trial Duration and Goal Width.
...
MAY also require users to specify "how quickly" Trial Durations should increase.
  ...
  Refer to ["Compliance"](#compliance) for important Search Goal instances.
...
The only REQUIRED attribute is a list of Search Goals.
...
apart from one Controller Input instance.
  ...
  For example, Traffic Profile is conceptually configured on the Measurer by the Manager,
without explicit assistance of the Controller.

  The order of Search Goals in a list SHOULD NOT
have a big impact on Controller Output,
but MLRsearch implementations MAY base their behavior on the order
of Search Goals in a list.
...
It is the maximal value the Controller is allowed to select for Trial Loads.
...
required by some implementations of MLRsearch.
  ...
  If the Max Load is provided, the Controller MUST NOT select
Trial Loads larger than that value.

  In theory, each Search Goal could have its own Max Load,
but as all Trial Results are possibly affecting all Search Goals,
it makes more sense for a single Max Load to apply to all Search Goals.
...
It is the minimal value the Controller is allowed to use for Trial Loads.
...
as opposed to using a different value for each Search Goal.
  ...
  If the Min Load is provided, the Controller MUST NOT select
Trial Loads smaller than that value.
...
and the implementation can apply a relative Goal Width safely.
...
: A Load is called an Upper Bound if and only if it is classified
...
Trial Results within the time remaining till the Goal Duration Sum is reached.

  One Search Goal can have multiple different Loads
classified as its Upper Bounds.
While Search progresses and more Trials are measured,
any Load can become an Upper Bound in principle.
...
: A Load is called a Lower Bound if and only if it is classified
as such by a [Load Classification Code](#load-classification-code)
algorithm for the given Search Goal at the current moment of the Search.
...
while a smaller Load is an Upper Bound at the same time.
...
: A Load is called Undecided if it is currently
neither an Upper Bound nor a Lower Bound for the give Search Goal.
...
with different Goal Final Trial Durations.
...
Conversely, at the end of the Search, each Search Goal insatnce
...
: The Relevant Upper Bound is the smallest Load
...
: The Relevant Lower Bound is the largest Load
...
: If no Load had enough Low-Loss Full-Length Trials,
the Relevant Lower Bound MAY be non-existent.
...
for a Relevant Lower Bound if larger Loads were available for selection.
...
within the Goal Duration Sum.
  ...
  This quantity is RECOMMENDED to be used when evaluating repeatability
...
only the missing Relevant Upper Bound signals this instance is irregular.

  Similarly, if both Relevant Bounds exist, it is RECOMMENDED
to include them as Irregular Goal Result attributes
and let the Manager decide if they are too far apart for Test Report purposes.
...
just by examining the attributes present.
...
as an ordered list of Goal Results that in that case MUST appear in the exactly same order
as the list of their corresponding Search Goals as specified in the Controller Input instance.

  When the Search Result is expressed as a mapping, it MUST contain an
entry for every Search Goal supplied in the Controller Input.
...
for example the number of Trials performed and the total Search Duration.
...
for example developed in different programming languages.
...
When measuring the real SUT, it is RECOMMENDED to turn any severe deviation
...
in the shortest average Search Duration.
...
as long as the Goal Result instances compared are regular.
...
: The Manager MUST initialize the SUT (including constituent DUTs) and the Measurer
...
but it MUST report all applied attributes even if not tweaked.

  A human or automated *user* invokes the Manager once to launch a
...
the Controller Input and Controller Output instances are implied.
...
The following Search Goal instance SHOULD be used to make the corresponding
Search Result unconditionally compliant with {{Section 24 of RFC2544}}.
...
A non-zero Goal Exceed Ratio is not strictly disallowed, but it could
...
The rationale of such a Search is to repeat High-Loss Trials,
...
would still fit within the Goal Exceed Ratio of 50%.
...
multiple Goal Loss Ratios, and improves repeatability.
...
usually differing in their Goal Loss Ratios.
...
when the Search is started with only one Search Goal.
...
and the Relevant Lower Bound are used in the Goal Result.
...
A zero Goal Exceed Ratio means all Trials must have a Trial Loss Ratio
...
or a Lower Bound for that Search Goal.
...
also used for all Goal Final Trial Durations.
...
The clearest illustration for adopting a generalized throughput definition
...
e.g., a specific frames-per-second limit on the Network Interface Card (NIC), a common occurrence.
...
Reporting a throughput that exceeds the SUT's verified hard limit would be
counterintuitive. Therefore the {{RFC2544}} Throughput metric should
...
only a 100.0 Mfps Trial Forwarding Rate, nicely confirming the known limitation.
...
than the Goal Loss Ratio of the next higher loss Search Goal.
...
and ["Loss Inversion"](#loss-inversion)
...
With respect to a particular Search Goal, the possibilities
...
Short Trials are slightly less likely to become High-Loss according to the Goal Loss Ratio,
...
Any Load value can be classified,
...
Only one Load is in focus, its value is one million frames per second (1 Mfps).

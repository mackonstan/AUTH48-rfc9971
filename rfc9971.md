...
(COTS) CPU hardware vs purpose-built ASIC
(Application-Specific Integrated Circuit) / NPU (Network Processing
Unit) / FPGA (Field-Programmable Gate Array) hardware.
...
with bridge MAC (Media Access Control) address learning being only one possible example.
...
An example standardization effort is described in {{I-D.ietf-bmwg-network-tester-cfg}}.
  ...
  * Data link frame size:
    ...
    - Fixed sizes as listed in {{Section 3.5 of RFC1242}} and in {{Section
      9 of RFC2544}}
    - IMIX ("Internet Mix") mixed frame sizes as defined in {{RFC6985}}
...
e.g., a specific frames-per-second limit on the NIC (Network Interface Card), a common occurrence.
...
Only one Trial Load is in focus, its value is one million frames per second (1Mfps).
